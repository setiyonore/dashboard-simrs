...
class PendapatanRanapController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('pendapatan.pendapatan_ranap', compact('modul', 'penjab'));
    }

    public function getPendapatanRanap(Request $request)
    {   
        ...
        if(request()->ajax()){
            ...
            dokter.nm_dokter AS dokter_register,
            (
            SELECT
                dokter.nm_dokter 
            FROM
		...
		SELECT bridging_sep.no_sep FROM bridging_sep WHERE bridging_sep.no_rawat=reg_periksa.no_rawat AND jnspelayanan='1' LIMIT 1
            ...
            ( SELECT SUM( totalbiaya ) FROM billing WHERE billing.no_rawat = reg_periksa.no_rawat ) AS total_rs,
        ...
        (
            (SELECT SUM(totalbiaya) FROM billing WHERE reg_periksa.no_rawat=billing.no_rawat AND billing.status ='Ralan Dokter Paramedis' AND nm_perawatan NOT IN ('PEMERIKSAAN/KONSUL DOKTER SPESIALIS (D)','PEMERIKSAAN/KONSUL DOKTER SPESIALIS (D)','PEMERIKSAAN/KONSULTASI DR SPESIALIS', 'PEMERIKSAAN/KONSULTASI DR UMUM/IGD', 'PEMERIKSAAN/ DOKTER SPESIALIS (D)', 'PEMERIKSAAN/KONSUL DOKTER IGD (D)', 'PEMERIKSAAN/KONSUL DOKTER SPESIALIS (DPJP 1) (D)', 'PEMERIKSAAN/KONSUL DOKTER SPESIALIS (KONSULAN) (D)', 'PEMERIKSAAN/KONSUL DOKTER UMUM (D)') LIMIT 1)
            -
            (SELECT SUM(totalbiaya) FROM billing WHERE reg_periksa.no_rawat=billing.no_rawat AND billing.status ='Ralan Dokter Paramedis' AND nm_perawatan NOT IN ('PEMERIKSAAN/KONSUL DOKTER SPESIALIS (D)','PEMERIKSAAN/KONSUL DOKTER SPESIALIS (D)','PEMERIKSAAN/KONSULTASI DR SPESIALIS', 'PEMERIKSAAN/KONSULTASI DR UMUM/IGD', 'PEMERIKSAAN/ DOKTER SPESIALIS (D)', 'PEMERIKSAAN/KONSUL DOKTER IGD (D)', 'PEMERIKSAAN/KONSUL DOKTER SPESIALIS (DPJP 1) (D)', 'PEMERIKSAAN/KONSUL DOKTER SPESIALIS (KONSULAN) (D)', 'PEMERIKSAAN/KONSUL DOKTER UMUM (D)') LIMIT 1)
        ...
        (
          (SELECT SUM(totalbiaya) FROM billing WHERE reg_periksa.no_rawat=billing.no_rawat AND `status`='Ranap Dokter Paramedis')
            ...
            ( SELECT SUM( detail_pemberian_obat.biaya_obat * detail_pemberian_obat.jml ) FROM detail_pemberian_obat WHERE detail_pemberian_obat.no_rawat = reg_periksa.no_rawat AND detail_pemberian_obat.`status`='Ralan' ) AS obat_ralan,
            ( SELECT SUM( detail_pemberian_obat.biaya_obat * detail_pemberian_obat.jml ) FROM detail_pemberian_obat WHERE detail_pemberian_obat.no_rawat = reg_periksa.no_rawat AND detail_pemberian_obat.`status`='Ranap' ) AS obat_ranap,
        ...
        ( SELECT SUM( detreturjual.subtotal ) FROM detreturjual WHERE ( LEFT ( detreturjual.no_retur_jual, 17 ))= reg_periksa.no_rawat ) AS retur_obat,
        ...
        ( SELECT SUM( totalbiaya ) FROM billing WHERE kamar_inap.no_rawat = billing.no_rawat AND `status`='Kamar' ) AS biaya_kamar,
        (
        ...
        ( SELECT SUM( totalbiaya ) FROM billing WHERE billing.no_rawat = reg_periksa.no_rawat AND STATUS = 'Tambahan' ) AS tambahan,
        ...
        $modul = $this->menu;
        return view('pendapatan.pendapatan_ranap', compact('modul'));
